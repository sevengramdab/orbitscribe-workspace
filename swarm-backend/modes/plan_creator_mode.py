"""
Plan Creator Mode — Interactive multi-option planning.

Instead of generating a single linear plan, this mode sketches 3-4 different
approaches and lets the user pick one before execution begins.
"""
...
from core.model_router import router, select_model
...
class PlanCreator:
    ...
    async def create_plan(self, goal: str, model: str | None = None, temperature: float | None = None) -> CreatedPlan:
...
Return ONLY a JSON array. No markdown, no explanations outside the JSON."""
        ...
        selected = model or await select_model(prefer_local=False)
        raw = await router.chat(messages, model=selected, temperature=temperature or 0.4)
...
# Global singleton
_plan_creator: Optional[PlanCreator] = None


def get_plan_creator() -> PlanCreator:
    global _plan_creator
    if _plan_creator is None:
        _plan_creator = PlanCreator()
    return _plan_creator
