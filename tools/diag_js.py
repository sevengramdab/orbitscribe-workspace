#!/usr/bin/env python3
"""Diagnose JS state via Chrome console."""
import sys
import os
import time
...
OUTPUT_PATH = sys.argv[1] if len(sys.argv) > 1 else os.path.join(
    os.path.dirname(__file__), "..", "screenshots", "jukebox_diag.png"
)
...
def main():
    ...
    pyautogui.typewrite("typeof InfiniteJukeboxFluid", interval=0.01)
    pyautogui.keyDown('return')
    pyautogui.keyUp('return')
    time.sleep(0.3)

    pyautogui.typewrite("typeof window.jukeboxFluid", interval=0.01)
    pyautogui.keyDown('return')
    pyautogui.keyUp('return')
    time.sleep(0.3)
