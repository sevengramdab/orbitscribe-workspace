#!/usr/bin/env python3
"""Manually init and start the jukebox engine via console."""
import sys
import os
import time
...
OUTPUT_PATH = sys.argv[1] if len(sys.argv) > 1 else os.path.join(
    os.path.dirname(__file__), "..", "screenshots", "jukebox_started.png"
)
...
def main():
    ...
    # Init and start
    pyautogui.typewrite("initInfiniteJukebox(); window.jukeboxFluid.start();", interval=0.01)
    pyautogui.keyDown('return')
    pyautogui.keyUp('return')
    time.sleep(0.5)

    pyautogui.typewrite("if(window.jukeboxParticles) window.jukeboxParticles.start();", interval=0.01)
    pyautogui.keyDown('return')
    pyautogui.keyUp('return')
    time.sleep(0.5)
    ...
    print("Waiting 10 seconds for fluid...")
    time.sleep(10.0)
