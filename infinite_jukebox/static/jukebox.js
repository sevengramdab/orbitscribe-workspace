...
class InfiniteJukeboxFluid {
    ...
    _initShaders() {
        ...
        const splatFrag = `
            ...
            void main() {
                ...
                // Soft injection — thin film buildup
                vec4 splat = vec4(u_color * 0.45, 1.0) * falloff;
        ...
        const displayFrag = `
            ...
            uniform float u_exposure; uniform float u_gamma;
            uniform float u_bloomIntensity; uniform sampler2D u_bloom;
            uniform vec3 u_bgColor; uniform float u_bgPattern;
            ...
            void main() {
                ...
                // Dark, moody background with kelvin tint
                vec3 bg = (u_bgColor + u_bgPattern * sin(v_uv.xyx * 3.14159)) * kelvinToRGB(u_colorTemp);

                // Hard cap + gentle curve: prevents white blowout, keeps color
                dye = min(dye, vec3(0.4));
                dye = 1.0 - exp(-dye * 1.5);
                float dyeIntensity = length(dye);
                // Dye tints the bg — bg always shows through for depth
                vec3 color = mix(bg, dye, min(0.6, dyeIntensity * 0.8));
                color += bloom;
                color *= u_exposure;

                // Crisp, unfiltered output — no desaturation or tone mapping
        ...
        const bloomPrefilterFrag = `
            ...
            void main() {
                ...
                // Moderate threshold — only the brightest oily sheen gets bloom
                vec4 b = max(c - 0.6, 0.0) * 2.5;
                gl_FragColor = b;
            }
        `;
    ...
    _disposeFramebuffers() {
        const gl = this.gl;
        const dispose = (obj) => {
            if (obj && obj.texture) { gl.deleteTexture(obj.texture); gl.deleteFramebuffer(obj.fbo); }
        };
        const disposeDouble = (dfbo) => {
            if (!dfbo) return;
            dispose(dfbo.read());
            dispose(dfbo.write());
        };
        ...
        disposeDouble(this.bloom);
        dispose(this.divergence);
        dispose(this.curl);
        this.velocity = this.density = this.pressure = this.bloom = null;
        this.divergence = this.curl = null;
    }
    ...
    _createFBO(w, h, channels, type, filtering) {
        ...
        if (status !== gl.FRAMEBUFFER_COMPLETE) {
            ...
            if (type === gl.UNSIGNED_BYTE) {
                console.error('[Jukebox] FBO incomplete even with UNSIGNED_BYTE. WebGL may not be functional.');
                return { texture: null, fbo: null, width: w, height: h };
            }
    ...
    applyConfig(config) {
        ...
        if (config.densityDissipation !== undefined) this.densityDissipation = config.densityDissipation;
        if (config.velocityDissipation !== undefined) this.velocityDissipation = config.velocityDissipation;
        ...
        // Rebuild framebuffers if resolution changed
        const newDyeRes = config.dyeResolution !== undefined ? config.dyeResolution : this.dyeResolution;
        const newSimRes = config.simResolution !== undefined ? config.simResolution : this.simResolution;
        if (newDyeRes !== this.dyeResolution || newSimRes !== this.simResolution) {
            this.dyeResolution = newDyeRes;
            this.simResolution = newSimRes;
    ...
    render() {
        ...
        // Bloom prefilter + blur (skipped when disabled for crisp, unfiltered output)
        if (this.bloomIntensity > 0.0) {
        ...
        this._blit(this.progDisplay, {
            ...
            u_bgColor: this._themeBgColor || [0.015, 0.02, 0.035],
            u_bgPattern: this._themeBgPattern || 0.01,
        });
    }
    ...
    _bindPointerEvents() {
        ...
        const _unlockAudio = () => {
            if (window.jukeboxAudio && window.jukeboxAudio.ctx && window.jukeboxAudio.ctx.state === 'suspended') {
                window.jukeboxAudio.ctx.resume().catch(() => {});
            }
            c.removeEventListener('mousedown', _unlockAudio, { capture: true });
            c.removeEventListener('touchstart', _unlockAudio, { capture: true });
        };
        c.addEventListener('mousedown', _unlockAudio, { capture: true, once: true });
        c.addEventListener('touchstart', _unlockAudio, { capture: true, once: true });
        ...
        window.addEventListener('mousemove', (e) => {
            if (!this.pointers[0] || !this.pointers[0].down) return;
            const [x, y] = getUV(e);
        ...
        // Touch support — keep array tidy by reusing slots and removing stale entries
        const getTouchIndex = (id) => this.pointers.findIndex(p => p.id === id);

        c.addEventListener('touchstart', (e) => {
            e.preventDefault();
            for (let t of e.changedTouches) {
                const idx = getTouchIndex(t.identifier);
                const [x, y] = getUV(t);
                if (idx >= 0) {
                    this.pointers[idx] = { id: t.identifier, x, y, dx: 0, dy: 0, down: true };
                } else {
        ...
        c.addEventListener('touchend', (e) => {
            for (let t of e.changedTouches) {
                const idx = getTouchIndex(t.identifier);
                if (idx >= 0) this.pointers[idx].down = false;
            }
            // Prune released touches to keep the array small
            this.pointers = this.pointers.filter(p => p.id === -1 || p.down);
        });
    ...
    feedAudioFrame(frame) {
        ...
        // Track peak energy for HUD indicator
        let peakEnergy = 0;
        for (let i = 0; i < n; i++) peakEnergy = Math.max(peakEnergy, bins[i]);
        window._lastAudioPeak = peakEnergy;
        const status = document.getElementById('status');
        if (status && peakEnergy > 0.01) {
            const bars = Math.round(peakEnergy * 8);
            status.textContent = '● Live ' + '█'.repeat(bars) + '░'.repeat(Math.max(0, 8 - bars));
        }
        ...
        const themeColors = this._themeAudioColors;
        const channels = [
            { range: [0.0, 0.08], color: themeColors ? themeColors[0] : [0.9, 0.2, 0.1] },   // Bass
            { range: [0.08, 0.16], color: themeColors ? themeColors[1] : [0.8, 0.5, 0.1] },  // Low mid
            { range: [0.16, 0.35], color: themeColors ? themeColors[2] : [0.2, 0.8, 0.3] },  // Mid
            { range: [0.35, 0.60], color: themeColors ? themeColors[3] : [0.2, 0.5, 0.9] },  // High mid
            { range: [0.60, 1.00], color: themeColors ? themeColors[4] : [0.6, 0.2, 0.9] },  // Treble
        ];
    ...
    resize() {
        ...
        // Fallback to window dimensions if canvas hasn't been laid out yet
        ...
        const sizeChanged = this.canvas.width !== displayW || this.canvas.height !== displayH;
        ...
        if (sizeChanged) {
            ...
            this.dyeResolution = Math.min(maxTex, Math.max(1024, maxDim));
            this.simResolution = Math.min(maxTex, Math.max(512, Math.round(maxDim * 0.5)));
            this._disposeFramebuffers();
            this._initFramebuffers();
        }
    }
...
function initInfiniteJukebox() {
    ...
    const particles = particleCanvas ? new ParticleSystem(particleCanvas) : null;
    if (particles) {
        particles.setFluid(fluid);
        window.jukeboxParticles = particles;
    }

    // Apply default theme (Aurora — bright, vibrant, high-res)
    applyVisualizerTheme(fluid, particles, 'aurora');
