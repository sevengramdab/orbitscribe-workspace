...
===========================================================
Imagine you are the master electrician wiring a smart mansion with
separate service panels (one per GPU). Each panel feeds a different
...
class FboDescriptor:
    ...
    dirty: bool = True
    # Which GPU panel owns this circuit — like labeling each breaker
    # with the panel number so the apprentice knows where to go.
    gpu_owner: int = 0
...
class GpuComputeManager(GpuComputeBackend):
    """
    Abstracts GPU dispatch across the detected GPU cluster.
