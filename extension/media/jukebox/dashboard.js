...
class MainBreakerDashboard {
    constructor() {
        ...
        this.sliders = {
            ...
            curl: document.getElementById('slider-curl'),
        ...
        this.readouts = {
            ...
            curl: document.getElementById('val-curl'),
    ...
    _bindSliders() {
        const scales = {
            ...
            curl: { min: 0, max: 100, div: 1, fixed: 0 },
            pressure: { min: 10, max: 60, div: 1, fixed: 0 },
            colortemp: { min: 2000, max: 10000, div: 1, fixed: 0, suffix: ' K' },
            gamma: { min: 1.0, max: 3.0, div: 10, fixed: 1 },
    ...
    _applyLiveConfig(key, value) {
        ...
        switch (key) {
            ...
            case 'frequency':
                ...
                break;
            case 'curl':
                if (fluid) fluid.applyConfig({ curlStrength: value });
                break;
            case 'pressure':
                if (fluid) fluid.applyConfig({ pressureIterations: value });
                break;
            case 'colortemp':
                if (fluid) fluid.applyConfig({ colorTemp: value });
                break;
            case 'gamma':
                if (fluid) fluid.applyConfig({ gamma: value });
                break;
            case 'bloom':
    ...
    // ========================================================================
    // Visualizer Theme — like swapping the whole cockpit paint scheme
    // ========================================================================
    _bindThemeSelect() {
        if (!this.themeSelect) return;
        this.themeSelect.addEventListener('change', () => {
            const themeName = this.themeSelect.value;
            const fluid = window.jukeboxFluid;
            const particles = window.jukeboxParticles;
            applyVisualizerTheme(fluid, particles, themeName);

            // Sync sliders to match theme
            const theme = VISUALIZER_THEMES[themeName];
            if (theme) {
                this._syncSlider('curl', theme.curlStrength, 0, 100);
                this._syncSlider('pressure', theme.pressureIterations, 10, 60);
                this._syncSlider('colortemp', theme.colorTemp, 2000, 10000);
                this._syncSlider('gamma', theme.gamma, 1.0, 3.0);
                this._syncSlider('bloom', theme.bloomIntensity, 0, 1);
                this._syncSlider('sunrays', theme.sunraysWeight, 0, 2);
                this._syncSlider('dither', theme.ditherStrength, 0, 0.1);
            }
    ...
    _syncSlider(key, value, min, max) {
        ...
        const scales = {
            curl: { div: 1, fixed: 0 },
            pressure: { div: 1, fixed: 0 },
            colortemp: { div: 1, fixed: 0, suffix: ' K' },
            gamma: { div: 10, fixed: 1 },
    ...
    async _sendConfig() {
        const payload = {
            ...
            curlStrength: parseFloat(this.sliders.curl.value),
            pressureIterations: parseInt(this.sliders.pressure.value, 10),
            colorTemp: parseInt(this.sliders.colortemp.value, 10),
            gamma: parseFloat(this.sliders.gamma.value) / 10,
            ...
            theme: this.themeSelect ? this.themeSelect.value : 'aurora',
        };
    ...
    _startStatusPolling() {
        this.statusInterval = setInterval(async () => {
            try {
                ...
                // Also update GPU bars from status poll
                if (Array.isArray(data.gpu_utilization)) {
                    this._ensureGpuBars(data.gpu_utilization.length);
    ...
    // ========================================================================
    _ensureGpuBars(count) {
        if (!this.gpuList) return;
        const current = this.gpuList.querySelectorAll('.gpu-row').length;
    ...
    _handleStreamData(data) {
        const gpuCount = Array.isArray(data.gpu_utilization) ? data.gpu_utilization.length : 0;
        if (gpuCount > 0) {
            this._ensureGpuBars(gpuCount);
            data.gpu_utilization.forEach((pct, idx) => {
                if (idx >= this.gpuFills.length) return;
                const clamped = Math.max(0, Math.min(100, pct));
                this.gpuFills[idx].style.width = `${clamped}%`;
